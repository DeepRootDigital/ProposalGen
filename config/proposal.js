 ...
 */
 ...
 exports.list = function(req, res, next) {
  Proposal.find({},'proposalname',function(err,proposals){
 ...
 */
 ...
 exports.load = function(req, res, next) {
  Proposal.find({proposalname: req.body.chosen}).exec(function(err,proposal){
    if (err) return next(err);
    ...
    for (var i=0;i<req.body.pages.length;i++) {
      if (req.body.pages[i].pagetype == "coverpage") {
        ...
        if (Math.floor(req.body.pages[i].pagetext) == 192) {
          doc.image("public/icons/"+pageimagetwo, Math.floor(req.body.pages[i].pagetext), 295, {width:492});
        } else {
          doc.image("public/icons/"+pageimagetwo, Math.floor(req.body.pages[i].pagetext), 295);
        }
...
function serviceTable(doc,tableinfo,discount) {
  ...
  .text("SERVICE ", 50, 210, {width:460, continued:true})
  ...
  for (var i=0;i<tableinfo.length;i++) {
    ...
    var startPos = 23 * (contentLength - tableinfo[i].pservice.length - 1) + 295;
    ...
    .text(tableinfo[i].pname, 60, startPos);
  ...
  if (discount != 0) {
    doc.fillColor('#7e1417').font('DinMedium').text('BMS Discount: ',{continued:true}).fillColor('#2c292a').font('DinLight').text("$"+discount);  
  }
  doc.fillColor('#7e1417').font('DinMedium').text('Grand Total: ',{continued:true}).fillColor('#2c292a').font('DinLight').text("$"+grandTotal);
}
...
function titleText(doc,title,text) {
  ...
  .stroke("#2c292a")
  .fontSize(30).fillColor('#2c292a').font('DinLight').text(title.toUpperCase(),125,190)
...
function titleTextSubList(doc,title,text,sub,list) {
  ...
  for (var i=0;i<list.length;i++) {
    if (list[i].text != ""){
      doc.text("»  "+list[i].text,{width:642});
    }
  }
}
