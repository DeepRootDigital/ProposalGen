<section class="screen">
	<?php include 'header.php'; ?>
	<div class="container text-container">
		<p>
			<img class="icon-img" src="images/servicepage-planning.png">
			<img class="icon-img" src="images/servicepage-branding.png">
			<img class="icon-img" src="images/servicepage-webdev.png">
			<img class="icon-img" src="images/servicepage-marketing.png">
			<img class="icon-img" src="images/servicepage-bd.png">
		</p>
		<h2 class="dark-treatment" style="display:block;"><span>//</span> Additional Services</h2>
		<h3 style="display:inline-block; border:none;">Our Multidisciplinary Approach</h3>
		<div>
			<p class="paragraph-edit">Business on Market St. engages in a multidisciplinary approach that guides all of our efforts on your behalf. We keep your overall business objectives in mind as we create and leverage scalable assets. Our efforts are integrated across all BMS departments to strengthen your strategic position in the marketplace. That integration directs how we put together just the right team of professionals for your project.</p>
			<div class="paragraph-edit-con"></div>
		</div>
	</div>
</section>

<section id="research-and-planning" class="screen discipline">
	<?php include 'header.php'; ?>
	<div class="sidebar-wrap">
		<img src="images/sidebar.png" class="sidebar">
	</div>
	<div class="container discipline-container">
		<h2 class="dark-treatment">Research and Planning</h2>
		<img class="icon" src="images/rd-icon.png">
		<div class="clearfix"></div>
		<div class="half-column">
			<p>
				<b>Planning</b>
			</p>
			<ul>
				<li>Due Dilligence Research</li>
				<li>*Feasibility Study</li>
				<li>*Business Plan Development</li>
				<li>*Marketing Plan Creation</li>
				<li>*Financial Modeling</li>
				<li>*Request for Proposal</li>
				<li>Distribution Plan</li>
			</ul>
			<p>
				<b>Consulting</b>
			</p>
			<ul>
				<li>Strategic Business Consulting</li>
				<li>*Investor Presentation Preperation</li>
			</ul>
		</div>
		<div class="half-column">
			<p>
				<b>Investor Grade Presentation</b>
			</p>
			<ul>
				<li>*Powerpoints/Animated Presentation</li>
				<li>*Executive Summary</li>
				<li>CSS 3</li>
			</ul>
			<p>
				<b>Market Research</b>
			</p>
			<ul>
				<li>Target Demographic</li>
				<li>SWOT Analysis</li>
				<li>Feasibility Analysis</li>
				<li>Competition Analysis</li>
				<li>Product / Services</li>
				<li>Distribution Points</li>
				<li>Business Development</li>
			</ul>
			<p>*Branded / Non Branded</p>
		</div>
	</div>
</section>

<section id="branding" class="screen discipline">
	<?php include 'header.php'; ?>
	<div class="sidebar-wrap">
		<img src="images/sidebar.png" class="sidebar">
	</div>
	<div class="container discipline-container">
		<h2 class="dark-treatment">Branding</h2>
		<img class="icon" src="images/brand-icon.png">
		<div class="clearfix"></div>
		<div class="half-column">
			<p>
				<b>Logo</b>
			</p>
			<ul>
				<li>Virtual ID</li>
				<li>Corporate ID</li>
			</ul>
			<p>
				<b>Branding Guide</b>
			</p>
			<ul>
				<li>Typography</li>
				<li>Primary & Secondary Colors</li>
			</ul>
			<p>
				<b>Stationary</b>
			</p>
			<ul>
				<li>Labels</li>
				<li>Letterhead</li>
				<li>Graphic Design</li>
			</ul>
		</div>
		<div class="half-column">
			<p>
				<b>Packaging</b>
			</p>
			<ul>
				<li>Labels</li>
				<li>3D Design</li>
				<li>Graphic Design</li>
			</ul>
		</div>
	</div>
</section>

<section id="web-development" class="screen discipline">
	<?php include 'header.php'; ?>
	<div class="sidebar-wrap">
		<img src="images/sidebar.png" class="sidebar">
	</div>
	<div class="container discipline-container">
		<h2 class="dark-treatment">Web Development</h2>
		<img class="icon" src="images/wd-icon.png">
		<div class="clearfix"></div>
		<div class="third-column">
			<p>
				<b>Ecommerce Website</b>
			</p>
			<ul>
				<li>Hybrid CMS</li>
				<li>Full Site</li>
				<li>3rd Party Integration</li>
			</ul>
			<p>
				<b>Lead Generation Website</b>
			</p>
			<ul>
				<li>Landing Pages</li>
				<li>Full Site</li>
			</ul>
		</div>
		<div class="third-column">
			<p>
				<b>Informational Website</b>
			</p>
			<ul>
				<li>Home</li>
				<li>About Us</li>
				<li>Services / Products</li>
				<li>Contact</li>
				<li>Blog</li>
			</ul>
			<p>
				<b>Technical Consulting</b>
			</p>
			<ul>
				<li>Web Apps</li>
				<li>Mobile Apps</li>
				<li>Social Media</li>
				<li>Design</li>
			</ul>
		</div>
		<div class="third-column">
			<p>
				<b>Feasibility & Consulting</b>
			</p>
			<ul>
				<li>Architecture</li>
				<li>Discovery</li>
				<li>Due Diligence</li>
				<li>3rd Party Integration</li>
				<li>Database Migration</li>
				<li>Platform Scalability Solutions</li>
			</ul>
		</div>
	</div>
</section>

<section id="marketing" class="screen discipline white">
	<?php include 'header.php'; ?>
	<div class="sidebar-wrap">
		<img src="images/sidebar.png" class="sidebar">
	</div>
	<div class="container discipline-container">
		<h2 class="white-treatment">Marketing</h2>
		<img class="icon" src="images/mk-icon.png">
		<div class="clearfix"></div>
		<div class="half-column">
			<p>
				<b>SEM</b>
			</p>
			<ul>
				<li>PPC</li>
				<li>Display</li>
				<li>Retargeting</li>
			</ul>
			<p>
				<b>SEO</b>
			</p>
			<ul>
				<li>Public Relations, Content Syndication, Outreach</li>
				<li>Backlinking</li>
				<li>Business Development, Partnership, Affiliates</li>
				<li>Split Testing</li>
				<li>Content Creation</li>
				<li>Webmastering, Analytics, Content Management, Technical SEO</li>
			</ul>
		</div>
		<div class="half-column">
			<p>
				<b>Technial Consulting</b>
			</p>
			<ul>
				<li>Web Apps</li>
				<li>Mobile Apps</li>
				<li>Feasibility Architecture, Discovery, Due Diligence</li>
				<li>Social Meida</li>
				<li>Design, Infographics, Multimedia</li>
			</ul>
			<p>
				<b>Packaging</b>
			</p>
			<ul>
				<li>Labels</li>
				<li>3D Designs</li>
				<li>Graphic Design</li>
			</ul>
		</div>
	</div>
</section>
<section id="marketing" class="screen discipline white">
	<?php include 'header.php'; ?>
	<div class="sidebar-wrap">
		<img src="images/sidebar.png" class="sidebar">
	</div>
	<div class="container discipline-container">
		<h2 class="white-treatment">Marketing [Cont.]</h2>
		<img class="icon" src="images/mk-icon.png">
		<div class="clearfix"></div>
		<div class="half-column">
			<p>
				<b>Print</b>
			</p>
			<ul>
				<li>POP</li>
				<li>Marketing Fliers, Postcards, Brochures</li>
			</ul>
			<p>
				<b>Market Research</b>
			</p>
			<ul>
				<li>Target Demographic</li>
				<li>SWAT Analysis</li>
				<li>Feasibility Analysis</li>
				<li>Product Services</li>
				<li>Competition Analysis</li>
				<li>Distribution Points</li>
				<li>Business Development</li>
			</ul>
		</div>
		<div class="half-column">
			<p>
				<b>On Location</b>
			</p>
			<ul>
				<li>Signage</li>
				<li>Fliers</li>
				<li>Booth Designs</li>
				<li>Trade Show Cards</li>
			</ul>
			<p>
				<b>Digital Media</b>
			</p>
			<ul>
				<li>Radio</li>
				<li>TV</li>
				<li>Multimedai Videography, Photography</li>
			</ul>
		</div>
	</div>
</section>

<section id="business-development" class="screen discipline white">
	<?php include 'header.php'; ?>
	<div class="sidebar-wrap">
		<img src="images/sidebar.png" class="sidebar">
	</div>
	<div class="container discipline-container">
		<h2 class="white-treatment">Business Development</h2>
		<img class="icon" src="images/bd-icon.png">
		<div class="clearfix"></div>
		<div class="half-column">
			<p>
				<b>Sales Support</b>
			</p>
			<ul>
				<li>Sales Team Development</li>
				<li>Phone Support</li>
				<li>Sales Pitch Creation</li>
				<li>Financial Modeling</li>
				<li>Request for Proposal</li>
				<li>Distribution Plan</li>
			</ul>
			<p>
				<b>Consulting</b>
			</p>
			<ul>
				<li>Strategic Business Consulting</li>
				<li>Investor Presentation Preparation</li>
			</ul>
		</div>
		<div class="half-column">
			<p>
				<b>New Streams of Revenue</b>
			</p>
			<ul>
				<li>Distribution</li>
				<li>Affiliates</li>
				<li>Partnerships</li>
			</ul>
		</div>
	</div>
</section>